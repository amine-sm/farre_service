...
exports.getUnreadContactRequestsCount =
  async (req, res) => {
    try {
      const [rows] =
        await pool.query(
      ...
      return res.status(500).json({
        success: false,
        message:
          "Impossible de charger le compteur.",
      });
    }
  };

exports.getContactRequestById = async (
  req,
  res
) => {
  try {
...
exports.updateContactRequestReadStatus =
  async (req, res) => {
    try {
      const id = Number(
        req.params.id
